(function (global) {
	var lang = {
		...
		// Array errors
		ARRAY_LENGTH_SHORT: "数组({schema.title||'未命名'})长度太短 ({data.length}), 最小长度 {schema.minItems}",
		ARRAY_LENGTH_LONG: "数组({schema.title||'未命名'})长度太长 ({data.length}), 最大长度 {schema.maxItems}",
		ARRAY_LENGTH_NOT_MATCH:"数组({schema.title||'未命名'})应有{schema.minItems}项, 当前有{data.length}项",
